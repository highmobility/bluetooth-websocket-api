...
export default class BTWSCore extends EventEmitter {
  ...
  requestDevice(data){
    let options = (data.name === undefined) ? {filters: [{services: [data.services]}]} : {filters: [{services: [data.services]}, {name: data.name}]};
    return navigator.bluetooth.requestDevice(options)
      .then(device => {
        ...
        if (debug) console.log('> DEVICE: ', this.devices[device.id]);
  ...
  connect(deviceId) {
    if (!this.devices[deviceId]) { return Promise.reject('Device is not connected.'); }
    ...
    return device.gatt.connect()
      .then(server => {
        ...
        this.emit('connected');
        if (debug) console.log('> SERVER: ', device.server);
      })
      .catch(error => {
  ...
  disconnect(deviceId){
    if (!this.devices[deviceId]) { return Promise.reject('Device is not connected.'); }
    if (!this.devices[deviceId].gatt.connected) { return Promise.reject('Device is already disconnected.'); }
    let device = this.devices[deviceId];
    ...
    if (debug) console.log('> Bluetooth Device connected: ' + device.gatt.connected);
  }
  ...
  onDisconnected(event) {
    ...
    this.emit('disconnected');
    if (debug) console.log('> Device is disconnected.');
  }
  ...
  getService(deviceId, serviceUuid){
    ...
    return device.server.getPrimaryService(serviceUuid)
      .then(service => {
        ...
        if (debug) console.log('> SERVICE:', device.services[serviceUuid]);
      })
      .catch(error => {
  ...
  getCharacteristic(deviceId, serviceUuid, characteristicUuid) {
    ...
    return service.getCharacteristic(characteristicUuid)
      .then(characteristic => {
        ...
        if (debug) console.log('>Get characteristic:', service.characteristics[characteristicUuid]);
      })
      .catch(error => {
  ...
  readValue(deviceId, serviceUuid, characteristicUuid) {
    ...
    return characteristic.readValue()
      .then(value => {
        ...
        if (debug) {
          console.log('> Read value (TypedArray) : ', (this.binaryToTypedArray(value)));
          console.log('> Read value (base64) : ', (this.bufferToBase64(this.binaryToTypedArray(value))));
        }
  ...
  handleNotifications(event) {
    ...
    if (debug) console.log('> [Notify] Value changed to: ', (this.bufferToBase64(this.binaryToTypedArray(value))));
  }
